...
QString CFormulaNode::to_mathml__template( long level, long& nodesCount )
{
	...
	while( (npos = str.indexOf( __YUNEN_TEMPLATE_MARK, npos )) != -1 )
	{
		...
		{
			str.remove( cb + 1, k );
			npos = cb + 1 + QString(__YUNEN_TEMPLATE_MARK).length();
		}
		else
			npos++;
	}
	...
	while ( (npos = (long) str.indexOf( _T('\n'), npos )) != -1 )
	{
		if( npos + 1 < str.length() && str.at(npos + 1) != QChar(0) )
		{
			...
			if( str.midRef(i, QString(__YUNEN_TEMPLATE_MARK).length()) != __YUNEN_TEMPLATE_MARK )
